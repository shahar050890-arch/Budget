...
window.Parser = (function () {
  ...
  function parse(raw) {
    ...
    if (/(הוראת קבע|הוראות קבע|הו"ק|תשלום קבוע|חיוב קבוע|מנוי חודשי)/.test(t)) {
      ...
      // משך ההוראה: "למשך 12 חודשים", "ל-6 תשלומים", "לשנה"
      const months = (function () {
        let m = text.match(/(?:למשך|במשך|ל-?)\s*(\d{1,3})\s*(?:חודשים|תשלומים|פעמים)/);
        if (m) return parseInt(m[1], 10);
        m = text.match(/(\d{1,3})\s*(?:חודשים|תשלומים|פעמים)/);
        if (m) return parseInt(m[1], 10);
        if (/למשך\s*שנה|לשנה אחת|שנה שלמה/.test(text)) return 12;
        if (/למשך\s*שנתיים/.test(text)) return 24;
        if (/חצי שנה/.test(text)) return 6;
        return null;
      })();
